...
volcano_plot <- function(top_table, plot_title, logfc_cutoff = NA, cutoff = 0.05, cutoff_column = "p_val_adj", log_column = "logFC", xlabel = expression(paste(log[2.0], " fold change"))) {
    ...
    p <- ggplot(top_table_filter, aes_string(x = log_column, y = "Log_Pvalue")) +
        ...
        geom_text_repel(label = top_table_filter$Symbol_new, color = muted("red"), segment.color = "black", box.padding = 0.5, min.segment.length = 0.0, size = 3.0, nudge_y = 0.25, seed = 12345L, family = "Noto Sans") +
...
all_samples_limma_tibble <- read_rds("../macrophage_clustering/all_samples_limma_tibble.rda")
...
figure_b <- volcano_plot(all_samples_limma_tibble[["Inflammatory macrophages"]], "Inflammatory macrophages")
figure_c <- volcano_plot(all_samples_limma_tibble[["Foam cells"]], "Foam cells")
figure_d <- volcano_plot(all_samples_limma_tibble[["MHC-hi macrophages"]], "MHC-hi macrophages")
figure_e <- volcano_plot(all_samples_limma_tibble[["cDCs"]], "Classical dendritic cells")

figure <- ggdraw() +
    draw_plot(figure_a, x = 0.01, y = 0.66, width = 0.99, height = 0.33)  +
    draw_plot(figure_b, x = 0.01, y = 0.33, width = 0.49, height = 0.33) +
    draw_plot(figure_c, x = 0.51, y = 0.33, width = 0.49, height = 0.33) +
    draw_plot(figure_d, x = 0.01, y = 0.0, width = 0.49, height = 0.33) +
    draw_plot(figure_e, x = 0.51, y = 0.0, width = 0.49, height = 0.33) +
    draw_label("A", size = 15.0, x = 0.0, y = 1.0, hjust = 0.0, vjust = 1.0, fontfamily = "Noto Sans", fontface = "bold") +
    draw_label("B", size = 15.0, x = 0.0, y = 0.66, hjust = 0.0, vjust = 1.0, fontfamily = "Noto Sans", fontface = "bold") +
    draw_label("C", size = 15.0, x = 0.5, y = 0.66, hjust = 0.0, vjust = 1.0, fontfamily = "Noto Sans", fontface = "bold") +
    draw_label("D", size = 15.0, x = 0.0, y = 0.33, hjust = 0.0, vjust = 1.0, fontfamily = "Noto Sans", fontface = "bold") +
    draw_label("E", size = 15.0, x = 0.5, y = 0.33, hjust = 0.0, vjust = 1.0, fontfamily = "Noto Sans", fontface = "bold")

figure_title <- ggdraw() +
    draw_label(label = "Figure 4", x = 0.5, hjust = 0.5, size = 20.0, fontfamily = "Noto Sans", fontface = "bold")

figure_grid <- plot_grid(figure_title, figure, ncol = 1L, rel_heights = c(0.05, 1.0))
ggsave("figure4.pdf", figure_grid, width = 12.0, height = 12.0, units = "in", device = cairo_pdf)
